    ...
    <footer className="bg-background border-t border-border pt-12 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 mb-12">
          {/* Main Contact */}
          <div>
            <h3 className={`text-2xl md:text-3xl font-bold text-foreground mb-2 ${isArabic ? 'font-arabic' : ''}`}>
              {isArabic ? 'لنبدأ معاً' : "Let's Create Something Great"}
            </h3>
            <p className={`text-muted-foreground mb-6 ${isArabic ? 'font-arabic' : ''}`}>
              {isArabic ? 'اتصل بنا اليوم ونخبرك بكيف يمكننا مساعدتك.' : 'Reach out today and discover how we can transform your digital presence.'}
            </p>
            <div className="space-y-3 text-sm">
              <div className="flex items-center gap-3">
                <span className="text-foreground font-semibold">✉</span>
                <a href="mailto:hello@nexusdesign.com" className="text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="flex flex-col justify-between">
            <div>
              <h4 className={`font-semibold text-foreground mb-4 ${isArabic ? 'font-arabic' : ''}`}>
              ...
              <div className="flex gap-3 mb-8">
                {socialLinks.map((link) => (
                  <a
                    ...
                    title={link.label}
                  >
                    <span className="text-sm font-bold group-hover:scale-110 transition-transform">{link.label.charAt(0)}</span>
                  </a>
                ))}
              </div>
            </div>
        ...
        <div className="text-center text-xs text-muted-foreground">
          <p className={isArabic ? 'font-arabic' : ''}>
            © {currentYear} {isArabic ? 'استوديو نيكسس للتصميم' : 'Nexus Design Studio'}. {isArabic ? 'جميع الحقوق محفوظة.' : 'All rights reserved.'}
          </p>
        </div>
      </div>
    </footer>
